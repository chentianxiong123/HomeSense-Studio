...
export async function getModels(): Promise<ModelsListResponse> {
  const raw = await request<ModelsListResponse & { pico?: ModelsListResponse }>("/api/models")
  // pi 引擎桥:响应里 pico.models 才是 PicoClaw 形状的 ModelInfo 列表
  if (raw.pico?.models) {
    return {
      ...raw.pico,
      provider_options: raw.provider_options ?? raw.pico.provider_options ?? [],
    }
  }
  return raw
}
