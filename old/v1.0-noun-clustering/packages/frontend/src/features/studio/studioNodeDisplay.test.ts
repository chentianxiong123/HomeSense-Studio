...
describe('studioNodeDisplay', () => {
  it('formats node labels and descriptions into localized display text', () => {
    ...
    expect(formatNodeDescription('device_capability', 'Device Capability', label)).toBe('通过统一设备能力执行动作，运行前会先进行沙箱演练。|Run a structured device capability with rehearsal before execution.')
