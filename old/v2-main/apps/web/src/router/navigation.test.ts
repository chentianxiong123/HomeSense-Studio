...
describe('normalizeRememberedRoute', () => {
  it('falls back to the chat route when storage is empty or invalid', () => {
  ...
  it('accepts supported first-class routes and nested Studio detail routes', () => {
    ...
    expect(normalizeRememberedRoute('/integrations')).toBe('/authorizations')
    expect(normalizeRememberedRoute('/streaming')).toBe('/authorizations')
...
describe('route storage constants', () => {
  it('exposes stable defaults for persisted navigation state', () => {
    expect(APP_DEFAULT_ROUTE).toBe('/chat')
