...
function summarizeDeviceCapabilityResult(
  ...
  const result = asRecord(outputs.result)
  if (!Object.keys(validation).length && !Object.keys(result).length) return null

  const hasValidation = Object.keys(validation).length > 0
  const validationData = asRecord(validation.data ?? validation)
  const resultData = asRecord(result.data ?? result)
  const device = buildDeviceSummary(validationData, resultData)
  const validationTone: SummaryTone = validationPassed(validation, validationData) ? 'success' : 'warning'
  ...
  const capability = stringifyValue(
    validationData.capability
      ?? validationData.capability_id
      ?? resultData.capability
      ?? resultData.capability_id,
  )
  const effect = stringifyValue(
    validationData.effect_summary
      ?? asRecord(validationData.projection).effect_summary
      ?? validationData.predicted_effect
      ?? validationData.next_step,
  )
  const changedFields = readChangedFields(validationData)
  const phases: NonNullable<WorkflowStepSummary['phases']> = []
  if (hasValidation) {
    phases.push({
      label: label('预览校验', 'Validation'),
      value: validationTone === 'success' ? label('通过', 'Passed') : label('阻塞', 'Blocked'),
      tone: validationTone,
    })
  }
  phases.push({
  ...
  return {
    ...
    tone: executionTone === 'error' ? 'error' : hasValidation ? validationTone : executionTone,
    device,
    phases,
    effect,
    changedFields,
    rows: compactRows([
      [label('设备 ID', 'Device ID'), stringifyValue(validationData.device_id ?? resultData.device_id ?? asRecord(validationData.device).id ?? asRecord(resultData.device).id)],
      [label('能力', 'Capability'), capability],
      [label('来源', 'Source'), stringifyValue(validationData.source ?? resultData.source)],
      [label('参数', 'Arguments'), stringifyValue(validationData.arguments ?? resultData.arguments)],
      [label('输出', 'Output'), stringifyValue(resultData.output)],
      [label('错误', 'Error'), stringifyValue(result.error ?? result.message ?? validation.error ?? validation.message)],
    ]),
  }
}
...
function buildDeviceSummary(
  ...
  const rawDevice = asRecord(validationData.device)
...
function validationPassed(validation: Record<string, unknown>, validationData: Record<string, unknown>): boolean {
  if (validation.status === 'error') return false
  if (validationData.ok === false || validationData.executable === false) return false
  return true
}

function readChangedFields(validationData: Record<string, unknown>): string[] {
  const projection = asRecord(validationData.projection)
  const fields: unknown[] = Array.isArray(validationData.changed_fields)
    ? validationData.changed_fields
