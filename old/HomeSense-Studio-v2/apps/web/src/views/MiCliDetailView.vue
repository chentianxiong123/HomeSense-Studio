...
import { api, type AuthStatus, type MiDeviceCandidate } from '@/api'
...
// Manual binding candidates
const devices = ref<MiDeviceCandidate[]>([])
...
async function doDiscover() {
  ...
  discoverError.value = ''
  try {
    const j = await api.userDevices.miCandidates()
...
<template>
  <div class="detail-page">
    <header class="page-head glass-panel">
      <button class="back-btn" @click="router.push('/authorizations')">← {{ label('返回', 'Back') }}</button>
      <div class="header-main">
        <span class="eyebrow">mi-cli</span>
        <h1>{{ label('mi-cli 登录与发现', 'mi-cli Auth & Discovery') }}</h1>
        <p>{{ label('米家账户授权与手动绑定候选。', 'Mi Home auth and manual binding candidates.') }}</p>
      </div>
      <div class="btn-group">
        ...
        <button v-if="loggedIn" class="primary-btn" @click="doDiscover" :disabled="discoverLoading">{{ label('读取候选', 'Load Candidates') }}</button>
    ...
    <section v-if="loggedIn" class="section-card glass-panel">
      <div class="section-head">
        <h3>{{ label('绑定候选', 'Binding Candidates') }}</h3>
      ...
      <div v-if="!devices.length && !discoverError" class="empty-action">
        <p>{{ label('点击上方"读取候选"按钮获取可手动绑定到设备的 MI did。', 'Click "Load Candidates" above to get MI did values for manual device binding.') }}</p>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="d in devices" :key="d.did">
              ...
              <td><span class="type-pill">{{ d.device_type }}</span></td>
              <td>{{ d.room_name || '-' }}</td>
            </tr>
          </tbody>
        </table>
