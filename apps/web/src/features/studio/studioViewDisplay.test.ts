...
describe('studioViewDisplay', () => {
  it('formats showcase labels and variable sources into localized display text', () => {
    ...
    expect(formatVariableSource('device_capability', label)).toBe('设备能力|Device Capability')
    expect(formatVariableSource('executor_call', label)).toBe('高级调用|Advanced Call')
  })
  ...
  it('formats execution modes, capability names, and binding kinds', () => {
    ...
    expect(formatCapability('dry_run', label)).toBe('预览|Preview')
    expect(formatCapability('adapter', label)).toBe('适配器|Adapter')
    expect(formatCapability('launch_app', label)).toBe('启动应用|Launch App')
    expect(formatBindingKind('a2a', label)).toBe('远程桥接|Remote Bridge')
