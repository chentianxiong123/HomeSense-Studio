import type { CLIBridge } from '../cli/cli-bridge'
...
const ADB_CAPABILITIES_PHONE: AdbCapabilityDefinition[] = [
  ...
  { id: 'adb.screenshot', name: '截屏', kind: 'property', adbAction: 'screenshot', input_schema: EMPTY_INPUT_SCHEMA, output_schema: null },
  { id: 'adb.current_app', name: '当前应用', kind: 'property', adbAction: 'current_app', input_schema: EMPTY_INPUT_SCHEMA, output_schema: null },
  { id: 'adb.ui_tree', name: '界面元素', kind: 'property', adbAction: 'ui_tree', input_schema: EMPTY_INPUT_SCHEMA, output_schema: null },
]
...
export async function buildDeviceCapabilityRegistry(
  ...
  if (device.adb_ip) {
    capabilities.push(...getAdbDefinitions(device.device_type).map(toAdbDeviceAgentCapability))
  }

  if (device.mi_did) {
    const result = await cliBridge.run('mi-cli', 'device_capabilities', { did: device.mi_did })
    if (result.status === 'success') {
      const miCaps = ((result.data as { capabilities?: Array<{ name: string; kind: string; type?: string }> })?.capabilities ?? [])
...
export function getAdbDefinitions(deviceType: string): AdbCapabilityDefinition[] {
  return deviceType === 'tv_box' || deviceType === 'television' || deviceType === 'stb'
    ? ADB_CAPABILITIES_TV_BOX
    : ADB_CAPABILITIES_PHONE
}

function toAdbDeviceAgentCapability(definition: AdbCapabilityDefinition): DeviceAgentCapability {
  return {
