...
// ProcessToAgent routes a message to a specific agent instance by ID,
// bypassing the static dispatch-rule resolver. Used by the v6 control plane
// for per-user agents. The agent is expected to already be registered (e.g.
// via EnsureUserAgent); if it is missing, an error is returned.
func (al *AgentLoop) ProcessToAgent(
	ctx context.Context,
