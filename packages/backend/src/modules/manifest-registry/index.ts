...
function buildCliManifests(): UnifiedExecutorManifest[] {
  return cliBridge.listExecutors().map((executor) => {
...
function buildAgentManifests(): UnifiedExecutorManifest[] {
  return agentAdapterRegistry.list().map((adapter) => {
...
function buildServiceManifests(): UnifiedExecutorManifest[] {
  const channelsByName = new Map(channelRegistry.list().map((channel) => [channel.name, channel]))
  return serviceRegistry.list().map((service) => {
    ...
    for (const [key, def] of Object.entries(fields)) {
      params_schema[key] = {
        type: typeof def.default === 'string' ? 'string' : 'unknown',
...
export const manifestRegistry = {
  list(): UnifiedExecutorManifest[] {
    return [...buildCliManifests(), ...buildAgentManifests(), ...buildServiceManifests()].sort(
