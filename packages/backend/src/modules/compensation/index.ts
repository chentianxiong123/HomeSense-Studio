...
export function createTask(
  ...
  eventBus.fire('compensation_task_created', { task_id: task.id, type })
  return task
}

export function persistState(task: CompensationTask): void {
  const db = getDb()
  db.prepare(
...
export function preview(task: CompensationTask): PreviewResult {
  ...
  if (task.type === 'device_control') {
    ...
    if (entityId) {
      ...
      if (entity) {
        ...
        const state = stateMachine.get(entityId)
...
export async function retryWithBackoff(task: CompensationTask): Promise<boolean> {
  if (task.retry_count >= task.max_retries) {
    task.state = 'failed'
    task.error_message = `已达最大重试次数 ${task.max_retries}`
    persistState(task)
    eventBus.fire('compensation_task_failed', { task_id: task.id, error: task.error_message })
    return false
  }
  ...
  try {
    ...
    if (task.type === 'device_control') {
      try {
        ...
        await serviceRegistry.call(serviceName, params)
        success = true
      } catch {}
    } else {
      try {
        await serviceRegistry.call(task.type, params)
        success = true
      } catch {}
    }
    ...
    if (success) {
      ...
      task.error_message = ''
      persistState(task)
      eventBus.fire('compensation_task_succeeded', { task_id: task.id })
      return true
    } else {
    ...
    persistState(task)
    eventBus.fire('compensation_retry', { task_id: task.id, retry_count: task.retry_count })
    return false
  }
}

export function getTask(id: number): CompensationTask | undefined {
  const db = getDb()
  return db.prepare('SELECT * FROM compensation_tasks WHERE id = ?').get(id) as CompensationTask | undefined
}

export function getPendingTasks(): CompensationTask[] {
  const db = getDb()
  return db.prepare(
...
export function processPendingTasks(): void {
  const tasks = getPendingTasks()
  for (const task of tasks) {
    retryWithBackoff(task).catch(() => {})
  }
}
