...
function stepLabel(): string {
  if (props.toolCall.name === 'rehearse_device_capability') return props.locale === 'zh' ? '沙箱演练' : 'Rehearsal'
...
function outcomeLabel(): string {
  ...
  if (tc.name === 'rehearse_device_capability') {
    const ok = tc.result?.ok !== false && tc.result?.executable !== false
    return ok ? (props.locale === 'zh' ? '通过' : 'Passed') : (props.locale === 'zh' ? '未通过' : 'Blocked')
  }
...
function isRehearsal(): boolean {
  return props.toolCall.name === 'rehearse_device_capability'
}
...
<template>
  <div :class="['tool-card', toolCall.status, { collapsed: !toolCall.expanded, rehearsal: isRehearsal() }]">
    ...
    <div v-show="toolCall.expanded" class="tool-body">
      ...
      <div v-if="isRehearsal() && !deviceCard" class="rehearsal-summary">
        <div class="rehearsal-head">
          <span :class="['rehearsal-dot', outcomeLabel() === '通过' || outcomeLabel() === 'Passed' ? 'success' : 'blocked']"></span>
          <strong>{{ locale === 'zh' ? '沙箱演练' : 'Sandbox rehearsal' }}</strong>
          <span>{{ outcomeLabel() }}</span>
        </div>
        <div v-if="toolCall.result?.predicted_effect" class="tool-device-note">{{ toolCall.result.predicted_effect }}</div>
        <div v-if="toolCall.result?.next_step" class="tool-device-note muted">{{ toolCall.result.next_step }}</div>
      </div>
...
.rehearsal-head span:last-child {
  font-size: 10px;
  font-weight: 900;
  color: var(--text-tertiary);
  letter-spacing: 0.08em;
...
.rehearsal-dot.success { background: #10b981; }
.capability-summary {
  margin-top: 12px;
  padding: 10px;
  border-radius: 8px;
