...
describe('buildStudioInspectorCopy', () => {
  it('provides localized labels for the inspector and executor sections', () => {
    ...
    expect(copy.deviceCapabilityConfig).toBe('设备能力配置|Device Capability Config')
    ...
    expect(copy.selectDevice).toBe('选择设备|Select device')
    expect(copy.selectCapability).toBe('选择能力|Select capability')
    expect(copy.inputSchema).toBe('输入结构|Input Schema')
    expect(copy.sampleDispatch).toBe('示例演练|Sample Rehearsal')
