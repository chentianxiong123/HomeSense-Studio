...
const ASSET_FILTER_KEY = 'homesense-studio.asset-filter'
const ASSET_SEARCH_KEY = 'homesense-studio.asset-search'
const SELECTED_ASSET_KEY = 'homesense-studio.selected-asset'
...
const filterOptions = computed(() => [
  ...
  { value: 'workflow', zh: '工作流', en: 'Workflows' },
...
async function loadAssets() {
  ...
  errorMessage.value = ''
  try {
    const [workflows, skills, manifests, plans, agents] = await Promise.all([
      workflowApi.list(),
      skillApi.list(),
      manifestApi.list(),
    ...
    errorMessage.value = (error as Error).message || 'Failed to load Studio assets.'
    assets.value = []
  } finally {
    loading.value = false
  }
...
function restorePreferences() {
  ...
  if (savedFilter === 'all' || savedFilter === 'workflow' || savedFilter === 'skill' || savedFilter === 'manifest' || savedFilter === 'plan' || savedFilter === 'agent') {
  ...
  <div class="studio-home">
    <section class="summary-strip">
      <div class="summary-card hero">
        <span class="summary-label">{{ label('Studio 资产中枢', 'Studio Asset Hub') }}</span>
        <strong>{{ label('统一资产工作台', 'Unified Asset Workbench') }}</strong>
        <p>{{ label('主页优先承载资产浏览、筛选、预览和跳转，工作流编排器退到详情二级页。', 'The homepage focuses on browse, filter, preview, and jump, while the workflow editor lives in a secondary detail page.') }}</p>
      </div>
      <div class="summary-card">
        <span class="summary-label">{{ label('工作流', 'Workflows') }}</span>
    ...
    <section class="studio-workbench">
      <aside class="asset-nav">
        <div class="nav-head">
          <h2>{{ label('资产中枢', 'Asset Hub') }}</h2>
          <button class="ghost-btn" @click="loadAssets">{{ label('刷新', 'Refresh') }}</button>
        </div>

        <div class="nav-caption">
          {{ label('统一承接 workflow、skill、manifest、plan、agent。', 'One entry surface for workflow, skill, manifest, plan, and agent.') }}
        </div>
      ...
      <main class="asset-table-area">
        <div class="asset-toolbar">
          <div>
            <h3>{{ label('统一资产表', 'Unified Asset Table') }}</h3>
            <p>{{ label('当前首页更偏向 Dify 式资产工作台，而不是直接把画布当首页。', 'This home surface follows a Dify-style asset workbench instead of making the canvas the landing page.') }}</p>
          </div>
          ...
          <table class="asset-table">
            ...
            <tbody>
              ...
              >
                ...
                <td class="asset-route">{{ asset.route }}</td>
                <td>
      ...
      <aside class="preview-panel">
        <template v-if="selectedAsset">
          ...
          <div v-if="selectedAsset.kind === 'workflow' && selectedAsset.workflowGraph" class="preview-card">
            <div class="preview-card-head">
              <h4>{{ label('节点图预览', 'Node Graph Preview') }}</h4>
              <span>{{ selectedAsset.workflowGraph.nodes.length }} {{ label('节点', 'nodes') }}</span>
            </div>
            <WorkflowGraphPreview :graph="selectedAsset.workflowGraph" compact />
          </div>
