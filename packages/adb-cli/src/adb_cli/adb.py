...
def handle_remove_files(params: dict) -> dict:
    ...
    _, stderr, code = _run_device_cmd(params, ["shell", f"rm -rf -- {targets}"], timeout=30)
    if code != 0:
...
def handle_copy_files(params: dict) -> dict:
    ...
    _, stderr, code = _run_device_cmd(params, ["shell", mkdir_cmd], timeout=15)
    if code != 0:
    ...
    for name in names:
        ...
        _, stderr, code = _run_device_cmd(params, ["shell", f"cp -R -- {src} {dst}/"], timeout=60)
        if code != 0:
...
def handle_pull_file(params: dict) -> dict:
    ...
    _, stderr, code = _run_device_cmd(params, ["pull", remote_path, local_path], timeout=120)
    if code != 0:
...
def handle_push_file(params: dict) -> dict:
    ...
    _, stderr, code = _run_device_cmd(params, ["shell", f"mkdir -p -- {shlex.quote(parent)}"], timeout=15)
    if code != 0:
        return {"status": "error", "error": "EXEC_FAILED", "message": stderr or "failed to create destination"}
    _, stderr, code = _run_device_cmd(params, ["push", local_path, remote_path], timeout=120)
    if code != 0:
