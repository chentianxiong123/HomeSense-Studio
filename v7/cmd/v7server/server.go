// The v7 control plane Server. It embeds picoclaw's AgentLoop and exposes a
...
// ServerConfig carries the static wiring for the v7 control plane.
...
// Server is the v7 control plane.
type Server struct {
	cfg     ServerConfig
	store   *Store
	...
	// sessions mints and resolves v7 login tokens.
...
func NewServer(cfg ServerConfig) (*Server, error) {
	...
	if cfg.GatewayBase != "" || cfg.GatewayKey != "" {
		pcCfg.ModelList = []*config.ModelConfig{{
			ModelName: cfg.Model,
			Provider:  "openai",
			Model:     cfg.Model,
			APIBase:   cfg.GatewayBase,
			APIKeys:   config.SimpleSecureStrings(cfg.GatewayKey),
		}}
	}
...
func (s *Server) handleChat(ctx context.Context, userID, content, sessionKey string) (string, error) {
	...
	return s.loop.ProcessToAgent(ctx, userID, content, sessionKey, "v7-api", userID)
}
